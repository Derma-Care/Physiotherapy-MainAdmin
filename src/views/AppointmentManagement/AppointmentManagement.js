      ...
      <div className="container mt-4">
        ...
        <div className="d-flex gap-2 mb-3">
          ...
          <button
            ...
            className={`btn ${filterTypes.includes('Video Consultation') ? 'btn-selected' : 'btn-unselected'
              }`}
          >
            Video Consultation
          </button>
            ...
            {loading ? (
              ...
              paginatedData.map((item, index) => (
                <CTableRow key={`${item.id}-${index}`}>
                  ...
                  <CTableDataCell> <CBadge style={{ backgroundColor: 'var(--color-black)', color: COLORS.white }}> {statusLabelMap[item.status] || item.status} </CBadge> </CTableDataCell>
